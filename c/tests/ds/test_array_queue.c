#include "alg/ds/array_queue.h"
#include "support/helper.h"

#define LEN 6
static const alg_elem_t DATA[LEN] = {0, 1, 2, 3, 4, 5};

AlgArrayQueue test_data(void) {
    return alg_array_queue_from_array(DATA, LEN);
}

void test_create(void) {
    AlgArrayQueue queue = alg_array_queue_create();
    char         *msg;

    msg = "should get a empty array queue";
    assert_eq(queue.len, 0, msg);
}

void test_from_array(void) {
    AlgArrayQueue queue = alg_array_queue_from_array(DATA, LEN);
    char         *msg;

    msg                 = "should create an array queue from an array";
    alg_elem_t tmp[LEN] = {0, 1, 2, 3, 4, 5};
    assert_eq(queue.len, LEN, msg);
    assert_arr_eq(queue.data, queue.len, tmp, LEN, msg);

    msg   = "should get an empty array queue when array is NULL";
    queue = alg_array_queue_from_array(NULL, LEN);
    assert_eq(queue.len, 0, msg);
}

void test_clear(void) {
    AlgArrayQueue queue = test_data();
    char         *msg;

    msg = "should clear";
    alg_array_queue_clear(&queue);
    assert_eq(queue.len, 0, msg);
    assert_eq(queue.front, 0, msg);
}

void test_is_empty(void) {
    AlgArrayQueue queue = test_data();
    char         *msg;

    msg = "should be empty when NULL";
    assert(alg_array_queue_is_empty(NULL), msg);

    msg = "should not be empty";
    assert_not(alg_array_queue_is_empty(&queue), msg);

    msg = "should be empty when len == 0";
    alg_array_queue_clear(&queue);
    assert(alg_array_queue_is_empty(&queue), msg);
}

void test_front(void) {
    AlgArrayQueue queue = test_data();
    alg_elem_t    e;
    char         *msg;

    msg = "should not get front when NULL";
    assert_not(alg_array_queue_front(NULL, NULL), msg);

    msg = "should get front";
    assert(alg_array_queue_front(&queue, &e), msg);
    assert_eq(e, queue.data[0], msg);

    msg = "should not get front when empty";
    alg_array_queue_clear(&queue);
    assert_not(alg_array_queue_front(&queue, NULL), msg);
}

void test_back(void) {
    AlgArrayQueue queue = test_data();
    alg_elem_t    e;
    char         *msg;

    msg = "should not get back when NULL";
    assert_not(alg_array_queue_back(NULL, NULL), msg);

    msg = "should get back";
    assert(alg_array_queue_back(&queue, &e), msg);
    assert_eq(e, queue.data[queue.len - 1], msg);

    msg = "should not get back when empty";
    alg_array_queue_clear(&queue);
    assert_not(alg_array_queue_back(&queue, NULL), msg);
}

void test_push_front(void) {
    AlgArrayQueue queue = alg_array_queue_create();
    alg_elem_t    e     = 999;
    char         *msg;

    msg = "should not push_front when NULL";
    assert_not(alg_array_queue_push_front(NULL, 0), msg);

    msg = "should push_front";
    assert(alg_array_queue_push_front(&queue, e), msg);
    assert_eq(queue.len, 1, msg);
    assert(alg_array_queue_push_front(&queue, e), msg);
    assert_eq(queue.len, 2, msg);

    msg       = "should not push_front when full";
    queue.len = ALG_MAX_LEN;
    assert_not(alg_array_queue_push_front(&queue, 0), msg);
    assert_eq(queue.len, ALG_MAX_LEN, msg);
}

void test_push_back(void) {
    AlgArrayQueue queue = alg_array_queue_create();
    alg_elem_t    e     = 999;
    char         *msg;

    msg = "should not push_back when NULL";
    assert_not(alg_array_queue_push_back(NULL, 0), msg);

    msg = "should push_back";
    assert(alg_array_queue_push_back(&queue, e), msg);
    assert_eq(queue.len, 1, msg);
    assert(alg_array_queue_push_back(&queue, e), msg);
    assert_eq(queue.len, 2, msg);

    msg       = "should not push_back when full";
    queue.len = ALG_MAX_LEN;
    assert_not(alg_array_queue_push_back(&queue, 0), msg);
    assert_eq(queue.len, ALG_MAX_LEN, msg);
}

void test_pop_front(void) {
    AlgArrayQueue queue = test_data();
    alg_elem_t    e;
    alg_elem_t    popped;
    char         *msg;

    msg = "should not pop_front when NULL";
    assert_not(alg_array_queue_pop_front(NULL, NULL), msg);

    msg    = "should pop_front";
    popped = queue.data[0];
    assert(alg_array_queue_pop_front(&queue, &e), msg);
    assert_eq(queue.len, LEN - 1, msg);
    assert_eq(e, popped, msg);

    msg = "should not pop_front when empty";
    alg_array_queue_clear(&queue);
    assert_not(alg_array_queue_pop_front(&queue, NULL), msg);
    assert_eq(queue.len, 0, msg);
}

void test_pop_back(void) {
    AlgArrayQueue queue = test_data();
    alg_elem_t    e;
    alg_elem_t    popped;
    char         *msg;

    msg = "should not pop_back when NULL";
    assert_not(alg_array_queue_pop_back(NULL, NULL), msg);

    msg    = "should pop_back";
    popped = queue.data[queue.len - 1];
    assert(alg_array_queue_pop_back(&queue, &e), msg);
    assert_eq(queue.len, LEN - 1, msg);
    assert_eq(e, popped, msg);

    msg = "should not pop_back when empty";
    alg_array_queue_clear(&queue);
    assert_not(alg_array_queue_pop_back(&queue, NULL), msg);
    assert_eq(queue.len, 0, msg);
}

int main(void) {
    char *mod    = "ds";
    char *target = "array_queue";

    run_test(test_create, mod, target, "create");
    run_test(test_from_array, mod, target, "from_array");
    run_test(test_is_empty, mod, target, "is_empty");
    run_test(test_front, mod, target, "front");
    run_test(test_back, mod, target, "back");
    run_test(test_push_front, mod, target, "push_front");
    run_test(test_push_back, mod, target, "push_back");
    run_test(test_pop_front, mod, target, "pop_front");
    run_test(test_pop_back, mod, target, "pop_back");

    return 0;
}
