...
interface LinkedList<T> {
  ...
  reduce<U>(fn: (acc?: U, val?: T, idx?: number) => U, init: U): U | undefined;
...
export const createLinkedList = <T>(...vals: T[]): LinkedList<T> => {
  ...
  const _checkIdx = (idx: number, closed: boolean = false) => {
    if (idx < 0 || idx >= (closed ? _len + 1 : _len)) {
      throw new Error("Index out of bounds");
    }
  };
  const _checkNode = (node: Node<T>) => {
    if (!node.prev && !node.next) {
      throw new Error("Cannot operate on detached node");
    }
  };
  const _getNode = (idx: number, closed: boolean = false): Node<T> => {
  ...
  const insert = (idx: number, val?: T) => {
    ...
    const target = _getNode(idx, true);
    _link(target, node);
  };
